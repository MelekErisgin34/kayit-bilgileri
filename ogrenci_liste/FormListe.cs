...
namespace ogrenci_liste
{
    public partial class FormListe : Form
    {
        ...
        string baglanti = "Server=localhost;Database=ogrenciler_birligi;Uid=root;Pwd=''";
        DataTable dt;
        ...
        void TumVerileriGetir()
        {
            using (MySqlConnection con = new MySqlConnection(baglanti))
            {
                ...
                string sql = "SELECT *FROM ogrenciler_liste";
                con.Open();
        ...
        private void btnSil_Click(object sender, EventArgs e)
        {
            using (MySqlConnection con = new MySqlConnection(baglanti))
            {
                string sql = "DELETE FROM ogrenciler_liste WHERE numara=@numara";
        ...
        private void btnGuncelle_Click(object sender, EventArgs e)
        {
            ...
            using (MySqlConnection con = new MySqlConnection(baglanti))
            {
                string sql = "UPDATE ogrenciler_liste SET numara=@numara, ad=@ad,soyad=@soyad,dtarih=@dtarih,cinsiyet=@cinsiyet,mezun_durum=@mezun_durum,WHERE numara=@numara;";
                int secilenNumara = Convert.ToInt32(txtNumara.Text);
        ...
        private void txtAra_TextChanged(object sender, EventArgs e)
        {
            ...
            if (rbAd.Checked)
            {
                sql = "SELECT *FROM ogrenciler_liste WHERE ad LIKE '%" + txtAra.Text + "%'";
            }

            else
            {
                sql = "SELECT *FROM ogrenciler_liste WHERE numara LIKE '%" + txtAra.Text + "%'";
            }
